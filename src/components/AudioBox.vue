<template>
  <div class="audio-box" :class="{ output: audioType === 'output' }">
    <div class="audio-box__wave">
      ...
      <div class="play-func-btns flex flex-x-between flex-y-center" v-show="readyState">
        <div class="flex gap-8">
          <i
            :class="['play-icon pointer', playState ? 'el-icon-video-pause' : 'el-icon-video-play']"
            data-sensors-click
            name="播放语音/暂停"
            @click="playPause"
          />
          <i class="el-icon-remove-outline play-icon pointer" @click="endedPlay" />
          <i
            class="el-icon-download play-icon pointer"
            data-sensors-click
            name="下载语音"
            @click="downloadAudio"
          />
        ...
        <div class="fs14 lh16">
          <label class="audio-box__wave-time" ref="timeRef" v-show="readyState">0:00</label>/<label
            class="audio-box__wave-duration asr-special-duration"
            ref="durationRef"
            v-show="readyState"
            >0:00</label
          >
...
export default {
  ...
  methods: {
    ...
    downloadAudio() {
      downloadFile(this.audioUrl, this.fileName || `audio_${Date.now()}.wav`)
    }
  },
  beforeDestroy() {
